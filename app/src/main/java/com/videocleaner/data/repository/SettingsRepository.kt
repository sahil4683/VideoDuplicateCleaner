...
class SettingsRepository @Inject constructor(
    ...
    val scanSettingsFlow: Flow<ScanSettings> = context.dataStore.data
        .catch { emit(emptyPreferences()) }
        .map { prefs ->
            ScanSettings(
                similarityThreshold = prefs[Keys.SIMILARITY_THRESHOLD] ?: 90,
                includeFolders = prefs[Keys.INCLUDE_FOLDERS]
                    ?.let { Json.decodeFromString<List<String>>(it) } ?: emptyList(),
                excludeFolders = prefs[Keys.EXCLUDE_FOLDERS]
                    ?.let { Json.decodeFromString<List<String>>(it) } ?: emptyList(),
                autoScanSchedule = prefs[Keys.AUTO_SCAN_SCHEDULE]
    ...
    val lastScanTimeFlow: Flow<Long> = context.dataStore.data
        .catch { emit(emptyPreferences()) }
